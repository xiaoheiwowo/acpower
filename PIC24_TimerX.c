...
int ADC_AN1[ADSAMPNUM] = {0}; //采样值保存数组
int ADC_AN0[ADSAMPNUM] = {0}; //采样值保存数组
int px = 0;                  //数组旧值保存位置标号
...
void __attribute__((__interrupt__, no_auto_psv)) _T1Interrupt(void)
{
    ...
    AN0Max = Max(ADC_AN0, ADSAMPNUM);
    AN0Min = Min(ADC_AN0, ADSAMPNUM);
    AN1Max = Max(ADC_AN1, ADSAMPNUM);
    AN1Min = Min(ADC_AN1, ADSAMPNUM);
